...
class Config:
    ...
    def config_totality_fix(self):
        ...
        logging.info("Начата проверка на целостность конфигов")
        try:
            ...
            for jf in os.listdir(self.this_dir):
                if jf.endswith('.json'):
                    ...
                    logging.info(f"Проверка целостности конфига: {jf}")
                    ...
                    if not os.path.exists(os.path.join(self.CONFIG_DIR, jf)):
                        ...
                        try:
                            ...
                            logging.info(f"Конфиг: {jf} вероятно повреждён\nПересоздаю конфиг")
                            ...
                            try:
                                ...
                                logging.debug(f"Конфиг {jf} был успешно пересоздан")

                            except Exception as Error:
                                print(f"Критическая ошибка при пересоздании конфига: \n{Error}")
                                logging.critical("Критическая ошибка при пересоздании конфига", exc_info=True)
                                time.sleep(5)
                                os.close(1)

                    if not fixed_config:
                        print(f"Конфиг {jf} полностью цел")
                        logging.info(f"Конфиг {jf} полностью цел")
                    else:
                        print(f"Конфиг {jf} успешно восстановлен")
                        logging.info(f"Конфиг {jf} успешно восстановлен")
                    os.system("cls")
                    os.system("clear")
            ...
            logging.error("Критическая ошибка проверки целостности конфигов", exc_info=True)
            time.sleep(5)
            os.close(1)
    ...
    def get_config(self, config_file: str, key):
        while 1:
            try:
                ...
                logging.info(f"Получены данные из {config_file}")
                logging.debug(f"Данные из {config_file}: {data}")
                return data[key]
            except Exception as Error:
                print(f"Ошибка при получении данных из конфига: \n{Error}\nЗапускаю проверку целостности конфигов")
                logging.error("Ошибка при получении данных из конфига\nЗапускаю проверку целостности конфигов", exc_info=True)
                os.system("cls")
                os.system("clear")